...
----------------------------------------------------------------------------------------------------------
-- The typing rules have to be proven first at the level of renamings to get weaken preservation properties

module Var where

  sucs-lemma : ∀ {m n Δ A i} (ρ : Ren m n)
               → Δ ⊢ var (suc i) ∈ (weaken $ A [ vars ρ ])
               → Δ ⊢ var (suc i) ∈ (A [ vars (map suc ρ) ])
  sucs-lemma {A = A} ρ ⊢i rewrite weaken-map-var A ρ = ⊢i

  map-suc-preserv : ∀ {m n Γ Δ A} (ρ : Ren m n)
                    → Δ ⊢ A
                    → Γ ⊢ vars ρ ∈s Δ
                    → Γ ⊢ vars (map suc ρ) ∈s Δ ∙ A
  map-suc-preserv []      ⊢A (⊢<> Δ⊢)        = ⊢<> (c-ext Δ⊢ ⊢A)
  map-suc-preserv (_ ∷ ρ) ⊢B (⊢<,> ⊢ρ ⊢A ⊢x) = ⊢<,> (map-suc-preserv ρ ⊢B ⊢ρ) ⊢A (sucs-lemma {A = ty-of ⊢A} ρ (ty-varₙ ⊢B ⊢x))

  id-preserv : ∀ {n} {Γ : Ctx n}
               → Γ ⊢
               → Γ ⊢ id ∈s Γ
  id-preserv {Γ = ⋄}     c-emp         = ⊢<> c-emp
  id-preserv {Γ = Γ ∙ _} (c-ext Γ⊢ ⊢A) = ⊢<,> (map-suc-preserv Ren.id ⊢A (id-preserv Γ⊢)) ⊢A (ty-q ⊢A)
  ...
  ↑-preserv : ∀ {m n Γ Δ A} {ρ : Ren m n}
              ...
              → Δ ⊢ A [ vars ρ ]
              → Γ ⊢ vars ρ ∈s Δ
              → Γ ∙ A ⊢ vars (Ren.↑ ρ) ∈s Δ ∙ (A [ vars ρ ])
  ↑-preserv {Δ = Δ} {A} {ρ} ⊢A ⊢A[ρ] ⊢ρ
    = ⊢<,> (map-suc-preserv ρ ⊢A[ρ] ⊢ρ) ⊢A
           (subst' {a = weaken (A [ vars ρ ])}
                   {A [ vars (map suc ρ) ]}
                   {Δ ∙ (A [ vars ρ ]) ⊢ q ∈_}
                   (trans (weaken-map-wk A (vars ρ))
                   (cong (A [_]) (map-var-weaken ρ)))
                   (ty-var₀ ⊢A[ρ]))

  var-ty-lemma : ∀ {m n x Δ i A} {ρ : Ren m n}
                 → Δ ⊢ x ∈ A [ vars ρ ]
                 → Δ ⊢ x ∈ (weaken A) [ vars (i ∷ ρ) ]
  var-ty-lemma {i = i} {A} {ρ} hyp
    rewrite weaken-sub-ext (var i) A (vars ρ) = hyp
  ...
  sub-ty-pr' {m = m} {n} {Δ = Δ} {ρ = ρ} (ty-Π-F {A = A} {B} ⊢A ⊢B) ⊢ρ
    = ty-Π-F (sub-ty-pr' ⊢A ⊢ρ) (subst' {a = vars (Ren.↑ ρ)} {↑ (vars ρ)}
                                        {λ x → (Δ ∙ (A [ vars ρ ])) ⊢ (B [ x ])}
                                        (sym $ ↑-map-var ρ)
                                        (sub-ty-pr' ⊢B (↑-preserv ⊢A (sub-ty-pr' ⊢A ⊢ρ) ⊢ρ)))
    
  sub-tm-pr' {ρ = x ∷ ρ} (ty-var₀ _)            (⊢<,> ⊢ρ ⊢A ⊢x) = var-ty-lemma {A = ty-of ⊢A} ⊢x
  sub-tm-pr' {ρ = x ∷ ρ} (ty-varₙ {A = A} _ ⊢i) (⊢<,> ⊢ρ ⊢B ⊢x) = var-ty-lemma {i = x} {A} $ sub-tm-pr' ⊢i ⊢ρ 
  sub-tm-pr' {Δ = Δ} {ρ = ρ} (ty-app {t = t} {A} {B} ⊢A ⊢B ⊢f ⊢t) ⊢ρ
    rewrite prop-β {σ = vars ρ} {t} {B}
      = ty-app (sub-ty-pr' ⊢A ⊢ρ)
               (subst' {a = vars (Ren.↑ ρ)} {↑ (vars ρ)}
                       {λ x → Δ ∙ (A [ vars ρ ]) ⊢ B [ x ]}
                       (sym $ ↑-map-var ρ)
                       (sub-ty-pr' ⊢B $ ↑-preserv ⊢A (sub-ty-pr' ⊢A ⊢ρ) ⊢ρ))
               (sub-tm-pr' ⊢f ⊢ρ)
               (sub-tm-pr' ⊢t ⊢ρ) 
  sub-tm-pr' {Δ = Δ} {ρ = ρ} (ty-Π-I {A = A} {B} {t} ⊢A ⊢B ⊢t) ⊢ρ
    = ty-Π-I (sub-ty-pr' ⊢A ⊢ρ)
             (subst' {a = vars (Ren.↑ ρ)} {↑ (vars ρ)}
                     ...
                     (sub-ty-pr' ⊢B $ ↑-preserv ⊢A (sub-ty-pr' ⊢A ⊢ρ) ⊢ρ))
             (subst' {a = vars (Ren.↑ ρ)} {↑ (vars ρ)}
                     {λ x → Δ ∙ (A [ vars ρ ]) ⊢ t [ x ] ∈ B [ x ]}
                     (sym $ ↑-map-var ρ)
                     (sub-tm-pr' ⊢t $ ↑-preserv ⊢A (sub-ty-pr' ⊢A ⊢ρ) ⊢ρ))

  -- These rules just wrap the fact that A [ map var ρ ] = A / ρ
  -- It was easier to prove them in the form of actual substitution first.
...
-- weakening types and terms preserves
...
subst-tm {γ = γ} (ty-app {f = f} {t} {A} {B} ⊢A ⊢B ⊢f ⊢t) ⊢γ
...
wfTm-wfTy {Γ = Γ} (ty-app {t = t} {A} ⊢A ⊢B ⊢f ⊢t) =
  let ⊢id = Var.id-preserv (wfTy-wf ⊢A)
  in subst-ty ⊢B (⊢<,> ⊢id ⊢A $ subst' {a = A}
                                      {A [ id ]}
                                      {Γ ⊢ t ∈_}
                                      (sym $ subId) ⊢t)
