...
-- Morphism from ΠU-ucwf of explicit substitutions to ΠU-ucwf of implicit substitutions
...
ΠExpSubUcwf⇒ : ΠU-ucwf-⇒ ExpSubΠUcwf ImpSubΠUcwf
ΠExpSubUcwf⇒ = record { λucwf-⇒     = λExpSubUcwf⇒
                      ; U-preserved = refl~βη
                      ; Π-preserved = refl~βη
                      }

-- Renamings to explicit substitutions
...
-- Normalized form of explicit p⋆, namely, a sequence of weakenings 

p⋆-norm : ∀ {n} → Sub-cwf (suc n) n
p⋆-norm {n} = varsExp Ren.p

-- p⋆ is convertible to its normalized form
...
-- id is preserved, follows by the former lemma
...
-- weakening a substitution is preserved
...
∘-preserved : ∀ {m n k} (γ₁ : Sub-λ k n) (γ₂ : Sub-λ m k) → ⟪ γ₁ ∘ γ₂ ⟫' ≈ ⟪ γ₁ ⟫' ∘⋆ ⟪ γ₂ ⟫'
...
-- Morphism from ΠU-ucwf of implicit substitutions to explicit substitutions
...
ΠImpSubUcwf⇒ : ΠU-ucwf-⇒ ImpSubΠUcwf ExpSubΠUcwf
ΠImpSubUcwf⇒ = record { λucwf-⇒     = λImpSubUcwf⇒
                      ; U-preserved = refl~
                      ; Π-preserved = refl~
                      }

-- Left inverse for terms and substitutions
...
-- Right inverse for terms and substitutions

right-inv-sub : ∀ {m n} (γ : Sub-λ m n) → ⟦ ⟪ γ ⟫' ⟧' ≈βη γ
right-inv-sub []      = refl≈βη
right-inv-sub (t ∷ γ) = ext (right-inv-tm t) (right-inv-sub γ)

-- Thus, we have an isomorphism of the two ΠU-ucwfs

ExpSubΠUcwf-ImpSubΠUcwf-≅ : ΠU-ucwf-≅ ΠExpSubUcwf⇒ ΠImpSubUcwf⇒
