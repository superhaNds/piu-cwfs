-------------------------------------------------------------------------------
-- A Ucwf of variables and renamings. Essentially a theory of n-place
-- projections with substitution formalized as a meta-level operation.
...
-------------------------------------------------------------------------------
-- Substitutions are vectors that contains Fin elements referred to as renamings (Ren)
...
-- generalized projection renaming
...
-- lookup in p returns the successor of the input index
...
-- This is ucwf of variables and renaming under propositional equality
...
ImpSubUcwf = record
               ...
               ; idL       = idL
               ; idR       = idR
               ; assoc     = λ {_}{_}{_}{_} {σ₁} {σ₂} {σ₃} → ∙-asso σ₁ σ₂ σ₃
               ; subId     = subId
               ; pCons     = p-∙
               ; qCons     = refl
